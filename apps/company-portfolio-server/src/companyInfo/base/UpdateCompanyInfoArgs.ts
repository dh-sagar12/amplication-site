/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CompanyInfoWhereUniqueInput } from "./CompanyInfoWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CompanyInfoUpdateInput } from "./CompanyInfoUpdateInput";

@ArgsType()
class UpdateCompanyInfoArgs {
  @ApiProperty({
    required: true,
    type: () => CompanyInfoWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CompanyInfoWhereUniqueInput)
  @Field(() => CompanyInfoWhereUniqueInput, { nullable: false })
  where!: CompanyInfoWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CompanyInfoUpdateInput,
  })
  @ValidateNested()
  @Type(() => CompanyInfoUpdateInput)
  @Field(() => CompanyInfoUpdateInput, { nullable: false })
  data!: CompanyInfoUpdateInput;
}

export { UpdateCompanyInfoArgs as UpdateCompanyInfoArgs };
